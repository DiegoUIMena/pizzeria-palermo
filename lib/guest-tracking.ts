...
/**
 * Genera un token seguro para que invitados puedan ver sus pedidos
...
export function generateGuestTrackingToken(
  ...
  // Usar una clave secreta (en producción, usar variable de entorno)
...
/**
 * Valida un token de seguimiento de invitado
...
/**
 * Extrae email y teléfono de un token sin validar la firma
 * (para debugging/logging solamente)
 */
export function extractGuestTokenData(token: string): {
  email?: string
  phone?: string
  ...
  try {
    const decoded = Buffer.from(token, 'base64').toString('utf-8')
    const [email, phone, expiryTimestampStr] = decoded.split('|')
    return {
      email,
      phone,
