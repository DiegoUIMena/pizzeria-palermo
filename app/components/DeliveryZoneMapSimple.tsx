...
export default function DeliveryZoneMapSimple({ isOpen, onClose, onSaveZones, initialZones }: DeliveryZoneMapProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Cargar mapa después de un momento
      setTimeout(() => {
        setMapLoaded(true)
  ...
  // Sincronizar polígonos con posición actual del mapa
  const syncPolygonsWithMap = useCallback(() => {
    console.log("🔄 SINCRONIZANDO POLÍGONOS CON MAPA")
    ...
    toast({
      title: "Polígonos sincronizados",
      description: `Se han actualizado ${zones.length} zonas con la posición actual del mapa`,
      duration: 2000
    })
  ...
  const finishDrawing = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `"${newZone.nombre}" se mantiene en ubicación exacta`,
        variant: "default"
      })
        ...
        <div className="flex h-[75vh] gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <div className="absolute top-2 left-2 z-30 bg-white rounded-lg border border-gray-200 p-2 shadow-sm">
              <div className="text-xs font-semibold mb-2">Navegación</div>
              <div className="grid grid-cols-3 gap-1">
                <div></div>
                <Button size="sm" variant="outline" onClick={() => moveMap('up')} className="h-6 w-6 p-0">↑</Button>
                <div></div>
                <Button size="sm" variant="outline" onClick={() => moveMap('left')} className="h-6 w-6 p-0">←</Button>
                <Button size="sm" variant="outline" onClick={() => setMapZoom(prev => Math.min(18, prev + 1))} className="h-6 w-6 p-0 text-xs">+</Button>
                <Button size="sm" variant="outline" onClick={() => moveMap('right')} className="h-6 w-6 p-0">→</Button>
                <div></div>
                <Button size="sm" variant="outline" onClick={() => moveMap('down')} className="h-6 w-6 p-0">↓</Button>
              ...
              {!isDrawing ? (
                <>
                  <Button 
                    ...
                    title="Sincronizar polígonos con la posición actual del mapa"
                  >
